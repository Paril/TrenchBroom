...
void FaceAttribsEditor::sinAnimationChanged(const QString& /* text */)
{
  ...
  {
    if (!setBrushFaceAttributes(
          map, {.sinAnimation = std::optional<std::string>{std::nullopt}}))
    {
      updateControls();
    }
  }
...
void FaceAttribsEditor::sinExtPhongGroupUnset()
{
  ...
  if (!setBrushFaceAttributes(
        map, {.sinExtPhongGroup = std::optional<int>(std::nullopt)}))
  {
    updateControls();
  }
}
...
void FaceAttribsEditor::createGui(gl::ContextManager& contextManager)
{
  ...
  m_surfaceSiNAnimationUnsetButton =
    createBitmapButton("ResetUV.svg", tr("Unset animation"));
  m_surfaceSiNAnimationEditorLayout = createUnsetButtonLayout(
    m_surfaceSiNAnimationEditor, m_surfaceSiNAnimationUnsetButton);
...
void FaceAttribsEditor::bindEvents()
{
  ...
  connect(
    m_surfaceSiNAnimationEditor,
    &QLineEdit::textEdited,
    this,
    &FaceAttribsEditor::sinAnimationChanged);
  connect(
    m_surfaceSiNAnimationUnsetButton,
    &QAbstractButton::clicked,
    this,
...
void FaceAttribsEditor::updateControls()
{
  ...
  const auto blockSiNExtPhongGroupEditor =
    QSignalBlocker{m_surfaceSiNExtPhongGroupEditor};
  ...
  if (!faceHandles.empty())
  {
    ...
    setValueOrMulti(
      m_surfaceSiNExtPhongGroupEditor,
      sinExtPhongGroupMulti,
      double(sinExtPhongGroup.value_or(0)));
